...
type YinboSpider struct {
	ProxyUrl string
	Cookies  string
}

// NewYinboSpider 创建 Yinbo 爬虫实例
func NewYinboSpider() *YinboSpider {
	return &YinboSpider{}
}

// SetProxy 设置代理地址
func (s *YinboSpider) SetProxy(proxyUrl string) {
	s.ProxyUrl = proxyUrl
}

// SetCookies 设置 Cookies
func (s *YinboSpider) SetCookies(cookies string) {
	s.Cookies = cookies
}

// GetStreamUrl 获取直播流地址
func (s *YinboSpider) GetStreamUrl(targetUrl string) (*StreamInfo, error) {
	// 1. Extract Room ID
	// https://live.ybw1666.com/800005143
	...
	params.Set("currentUrl", fmt.Sprintf("https://wap.ybw1666.com/%s", roomId))

	api := fmt.Sprintf("https://wap.ybw1666.com/api/ui/room/v1.0.0/live.ashx?%s", params.Encode())
	...
	req.Header.Set("Referer", fmt.Sprintf("https://live.ybw1666.com/%s?promoters=0", roomId))
	if s.Cookies != "" {
