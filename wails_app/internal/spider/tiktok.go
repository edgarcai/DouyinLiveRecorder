...
func (t *TikTokSpider) GetStreamUrl(url string) (string, error) {
	// Headers
	...
	if len(matches) < 2 {
		return "", fmt.Errorf("SIGI_STATE not found in tiktok page")
	}

	jsonStr := matches[1]
	var data map[string]interface{}
	if err := json.Unmarshal([]byte(jsonStr), &data); err != nil {
		return "", fmt.Errorf("failed to parse SIGI_STATE: %v", err)
	}
	...
	if !ok {
		return "", fmt.Errorf("LiveRoom not found")
	}

	liveRoomUserInfo, ok := liveRoom["liveRoomUserInfo"].(map[string]interface{})
	if !ok {
		return "", fmt.Errorf("liveRoomUserInfo not found")
	}
	...
	return "", fmt.Errorf("no stream url found in tiktok data")
}
