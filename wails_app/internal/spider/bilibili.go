...
func (b *BilibiliSpider) GetStreamUrl(url string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("play data not found")
	}

	playUrlInfo, ok := data2["playurl_info"].(map[string]interface{})
	if !ok {
		return "", fmt.Errorf("playurl_info not found")
	}

	playurl, ok := playUrlInfo["playurl"].(map[string]interface{})
	if !ok {
		return "", fmt.Errorf("playurl object not found")
	}
	...
	finalUrl := fmt.Sprintf("%s%s%s", host, baseUrl, extra)
	return finalUrl, nil
}
