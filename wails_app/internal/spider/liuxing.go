...
type LiuxingSpider struct {
	ProxyUrl string
	Cookies  string
}

// NewLiuxingSpider 创建 Liuxing 爬虫实例
func NewLiuxingSpider() *LiuxingSpider {
	return &LiuxingSpider{}
}

// SetProxy 设置代理地址
func (s *LiuxingSpider) SetProxy(proxyUrl string) {
	s.ProxyUrl = proxyUrl
}

// SetCookies 设置 Cookies
func (s *LiuxingSpider) SetCookies(cookies string) {
	s.Cookies = cookies
}

// GetStreamUrl 获取直播流地址
func (s *LiuxingSpider) GetStreamUrl(targetUrl string) (*StreamInfo, error) {
	// 1. Extract Room ID
	// https://wap.7u66.com/123456
	...
	// 2. Call API
	params := url.Values{}
	params.Set("promoters", "0")
	params.Set("roomidx", roomId)
	params.Set("currentUrl", fmt.Sprintf("https://www.7u66.com/%s?promoters=0", roomId))

	api := fmt.Sprintf("https://wap.7u66.com/api/ui/room/v1.0.0/live.ashx?%s", params.Encode())
	...
	req.Header.Set("Referer", fmt.Sprintf("https://wap.7u66.com/%s?promoters=0", roomId))
	if s.Cookies != "" {
